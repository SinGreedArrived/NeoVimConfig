...
local function get_current_par()
	...
	local rowj = row
	while true do
		local lastLine = vim.api.nvim_buf_get_lines(0, rowj-1, rowj, false) or {""}

		if lastLine[1] == "" then
			break
		end

		if lastLine[1]:find("^#") ~= nil then
			break
		end
	...
	local name = table.concat(vim.api.nvim_buf_get_lines(0, rowj-1, rowj, false), " "):sub(2)

	return name, cmd
end

local function ExecCommand()
	local name,cmd = get_current_par()
	vim.cmd('tabnew '.. name ..' | r ! ' .. cmd)
end
